...
st.sidebar.markdown("**3. Video Export Resolution**")
...
if uploaded_file:
    ...
    # NEW: Three-column layout for our three actions
    ...
    # --- LOGIC BRANCH 1: AUDIO EXTRACTION ONLY ---
    if extract_mp3_only:
    ...
    # --- LOGIC BRANCH 2: AI TRANSCRIPTION & VIDEO RENDERING ---
        ...
        status_text.info("Phase 1/4: Loading AI Model into Intel CPU Memory...")
        progress_bar.progress(25)

        if selected_lang == "Akan (Twi)":
            pipe = load_akan_model()
            status_text.warning("Phase 2/4: Transcribing Akan Twi Audio (This takes a few minutes on CPU...)")
            ...
            status_text.warning("Phase 2/4: Transcribing Audio (This takes a few minutes on CPU...)")
            progress_bar.progress(50)
        ...
        if generate_srt_only:
            status_text.success("Phase 4/4: Complete! SRT Ready.")
            progress_bar.progress(100)
            with open(output_srt, "rb") as f:
                st.download_button("📝 Download .SRT File", f, file_name="subtitles.srt")

        if generate_and_burn:
            status_text.info(f"Phase 4/4: Burning styled captions onto video at {export_res}...")
            ...
            try:
                ...
                status_text.success("✅ Complete! Video Rendering Finished.")
                progress_bar.progress(100)
                ...
                status_text.error(f"Error burning subtitles: {e.stderr.decode('utf-8')}")

        # Cleanup temp files safely
        for f in [input_video, output_srt, output_mp3]:
